...
contract PublicGoodsDonator is OwnedClaimable {
    ...
    constructor(address _owner, IPreStake _preStake, IWFIL _wFIL, IERC20 _liquidStakingToken) OwnedClaimable(_owner) {
        preStake = _preStake;
        wFIL = _wFIL;
     ...
     */
    function deposit(address recipient, uint256 amount, uint256 donationPercent)
        external
        noOverDonations(donationPercent)
    {
     ...
     * @param donationPercent The percentage of the amount to split to the PG wallet. 100 = 100%
     */
    function deposit(address recipient, uint256 donationPercent) external payable noOverDonations(donationPercent) {
        // normalize the address in case of a FIL ID
        recipient = recipient.normalize();
        // track before and after balance to compute how many tokens we got back
        uint256 preDepositBal = liquidStakingToken.balanceOf(address(this));
    ...
    function _donateIFIL(address recipient, uint256 newIFIL, uint256 donationPercent) internal {
